...
def generate_random_territory_allocation(
    ...
    territory_list = list(starting_territories[territory_unit_column].unique())
    ...
    for player in player_list:
        ...
        if selected_territory not in territory_list:
            ...
            territory_list.append(selected_territory)

    # Keep going through the following process until all territory has been allocated
    while len(territory_list) < len(gdf[territory_unit_column].unique()):
    #while len(territory_list) < 100:
    ...
    if return_df:
        ...
        return pd.merge(gdf, owned_territory_df, on=territory_unit_column)
    else:
...
# Bisualise the historical boundaries
grouped_dispatcher_gdf_boundary.plot(
    ax=ax,
    linewidth=2,
    edgecolor="green"
