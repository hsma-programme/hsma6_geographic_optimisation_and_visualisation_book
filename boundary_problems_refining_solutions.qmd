...
def is_solution_continuous(solution_gdf):
    """
    Checks if all dispatcher's regions in the given GeoDataFrame are spatially contiguous.

    Args:
        solution_gdf (GeoDataFrame): The GeoDataFrame representing the solution.

    Returns:
        bool: True if all regions are continuous, False otherwise.
    """
    # Merge LSOAs within the same dispatcher into single polygons
    dissolved = solution_gdf.dissolve(by="centre_dispatcher_evolved")

    # If any dispatcher has multiple disjoint parts, return False
    return not dissolved.geometry.apply(lambda geom: geom.geom_type == "MultiPolygon").any()
```
...
def create_evolved_solutions(
    ...
    simplified_allocation_df = initial_solution_gdf[[random_solution_allocation_colname, 'LSOA11CD', border_colname, "geometry"]]
    ...
    for i in range(population_size):
        #continuous_solution = False

        #while not continuous_solution:
        evolved_df = simplified_allocation_df.copy(deep=True)

        evolved_df[new_allocation_colname] = simplified_allocation_df.apply(assign_new_dispatcher, axis=1)

        #continuous_solution = is_solution_continuous(evolved_df)
...
```{python}
#| label: generate-fifty-solutions
...
for i, df in enumerate(solution[:10]):
    ax = axes[i]
